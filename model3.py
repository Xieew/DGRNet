...
class RegNetWithAttention(nn.Module):
    def __init__(self, cfg: dict, in_c=3, num_classes=1000, zero_init_last_bn=True, dropout_prob=0.2,
        ...
        # 假设 stem_c 为 32（从配置中获取）
        mid_c = stem_c // 2  # 中间通道数，可根据需要调整比例
        self.stem = nn.Sequential(
            ConvBNAct(in_c, mid_c, kernel_s=3, stride=2, padding=1),  # 下采样
            ConvBNAct(mid_c, stem_c, kernel_s=3, stride=1, padding=1)  # 保持分辨率
        )
    ...
    def forward(self, x):
        ...
        for name, module in self.named_children():
            if name in ['stem', 'head'] or isinstance(module, (ConvBNAct, RegHead)):
                continue
            x = module(x)
        x = self.head(x)
        return x
